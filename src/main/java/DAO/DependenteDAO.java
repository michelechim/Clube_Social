...
public class DependenteDAO extends BaseDAO {
	public static boolean RegDep(Dependente dependente) {
		...
		(Connection conn = getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql);) {
			...
			pstmt.setString(4, dependente.getSocio());
	...
	// m�todo utilit�rio, converte ResultSet na classe de modelo (nesse caso,
	// Dependente)
	private static Dependente resultsetToDependente(ResultSet rs) throws SQLException {
		...
		d.setSocio(rs.getString("cartao_socio"));
//		d.setSocio(SocioDAO.selectSocioById(rs.getLong("cartao_socio")));
		return d;
	}

	public static void main(String[] args) {
		Dependente dependente = new Dependente(1l, "maria", "mãe", "maria@gmail.com", "5");
